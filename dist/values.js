...
export default nable.lazy({
    ...
    files: async () => await core
        .getInput("files")
        .nLet((it) => nable.parseList(it))
        .nLet((it) => utils.matchFiles(it))
        .then((files) => files.map((file) => ({ name: path.basename(file), path: file }))),
    name: () => core.getInput("name"),
    changelog: () => core.getInput("changelog").nLet((it) => {
        if (fs.existsSync(it)) {
            try {
    ...
    gameVersions: async () => {
        ...
        return core
            ...
            if (game_version.split(":").length === 1) {
                ...
                if (index === -1)
                    throw Error(`Invalid minecraft version: ${game_version}`);
                return [game_version];
            }
            else {
                const [start_game_version, end_game_version] = nable.parsePair(game_version);
                const start_index = minecraftVersions.indexOf(start_game_version);
                const end_index = minecraftVersions.indexOf(end_game_version);
                if (start_index === -1)
                    throw Error(`Invalid minecraft start version: ${start_game_version}`);
                if (end_index === -1)
                    throw Error(`Invalid minecraft end version: ${end_game_version}`);
                if (start_index > end_index)
                    throw Error(`Start version is greater than end version: ${game_version}`);
