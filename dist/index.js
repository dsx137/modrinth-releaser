...
export async function findVersion(projectId, versionNumber) {
...
export async function createVersion(data, files) {
...
export async function modifyVersion(versionId, data) {
...
export async function addFilesToVersion(versionId, files) {
...
export async function deleteVersionFiles(files) {
    await core.group(`🗑️ Delete [${files.length}] version files`, async () => {
        await Promise.all(files.map(async (file) => await net.fetchToModrinth("DELETE", `/version_file/${file.hashes.sha512}`).then(async (res) => {
            if (!res.ok)
                throw Error(`${res.status}: ${await res.text()}`);
